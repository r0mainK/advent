...
def update_pointers(pointer_delta: int):
    def decorator(op: Callable):
        def wrapper(*args):
            ...
            new_instruction_pointer = op(*args)
            if new_instruction_pointer is None:
                new_instruction_pointer = pointers["instruction"] + pointer_delta
            pointers["instruction"] = new_instruction_pointer
...
def acquire_input():
    program[pointers["parameter_1"]] = int(input("enter system ID: "))


@update_pointers(pointer_delta=2)
def output_signal() -> int:
    print("diagnostic code: %d" % program[pointers["parameter_1"]])
